...
export default function AddPassword() {
  ...
  async function submit(e) {
    ...
    try {
      if (editId) {
        await api(`/vault/${editId}`, { method: 'PUT', body: { website, username, password } });
        setMsg('Entry updated successfully.');
        setMsgType('success');
      } else {
        await api('/vault', { method: 'POST', body: { website, username, password } });
        setWebsite(''); setUsername(''); setPassword('');
        setMsg('Password saved and encrypted.');
        setMsgType('success');
      }
    } catch (e) {
      setMsg(e.message);
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <span className="page-subtitle">Stored encrypted — never in plain text</span>
      </div>
        ...
        <form onSubmit={submit}>
          ...
          <div style={{ marginTop: '1.4rem' }}>
            <button className="btn" disabled={loading}>
              🔒 {loading ? 'Saving…' : editId ? 'Update & Encrypt' : 'Encrypt & Save'}
            </button>
          </div>
        </form>
      </div>
